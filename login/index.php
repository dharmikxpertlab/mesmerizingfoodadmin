<?php 
    ...
    if(isset($_POST['login'])){
        $flag=1;
        $email=mysqli_real_escape_string($connection,trim($_POST['username']));
        $password=trim($_POST['password']);

        if($email=='')
		{
			$mobileNumberError="<div class='error'>Email Required</div>";
			$flag=0;
		}
		if($password=='')
		{
			$passwordError="<div class='error'>Password Required</div>";
			$flag=0;
		}
        ...
        if($flag==1)
		{
			...
			$admin=mysqli_query($connection,"select * from `admin` where `email`='$email' AND `password` = '".md5($password)."' AND `delete`= 0");	
...
<html lang="en" >
    ...
    <head>
                <meta charset="utf-8"/>
        <title>Mesmerizing Food | Login</title>
	...
	<div class="d-flex flex-center bgi-size-cover bgi-no-repeat flex-row-fluid" style="background-image: url(../assets/media/bg/bg-2.jpg);">
		<div class="login-form text-center text-white p-7 position-relative overflow-hidden">
			...
			<div class="d-flex flex-center mb-15">
				<a href="">
                    <h1 style="font-size: 40px;">Mesmerizing Food</h1>
			...
			<div class="login-signin">
				...
				<form class="form" method="POST" id="kt_login_signin_form">
					<div class="form-group">
						<input class="form-control h-auto text-white bg-white-o-5 rounded-pill border-0 py-4 px-8" type="text" id="email" placeholder="Email" name="username" autocomplete="off"/>
                        <?php if(isset($mobileNumberError)) { echo $mobileNumberError; } ?>
					</div>
					<div class="form-group">
						<input class="form-control h-auto text-white bg-white-o-5 rounded-pill border-0 py-4 px-8" type="password" id="password" placeholder="Password" name="password"/>
					...
					<div class="form-group text-center mt-10">
						<button id="kt_login_signin_" type="submit" name="login" class="btn btn-pill btn-primary opacity-90 px-15 py-3">Sign In</button>
					</div>
				</form>
