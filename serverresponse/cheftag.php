...
$aColumns = array("rowNumber", "Actions", "tagname","lastUpdate");
...
$sTable = "
		(
			SELECT
				...
				(
					SELECT
                        `cheftag`.`chefTagName`as tagname,
                       
                        CONCAT(`admin`.`adminName`,'<br>At: ',`cheftag`.`updated`) as `lastUpdate`,
                        
                        

						CONCAT('<div class=\"margin-bottom-5\"><a href=\"../addcheftag-',TO_BASE64(`cheftag`.`cheftagId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a><button onclick=\"remove(',`cheftag`.`cheftagId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button></div>') AS `Actions`
						
						FROM `cheftag` LEFT JOIN `admin` ON `cheftag`.`adminId`=`admin`.`adminId` where `cheftag`.`delete`= '0'
