...
$aColumns = array("rowNumber", "Actions", "fullName", "videoTitle", "videoName", "cuisineName", "name");
		...
		(
			SELECT
				...
				(
					SELECT
                        `chef`.`fullName`,
                        `chefvideo`.`videoTitle`,

                        CONCAT('<video width=\"200\" controls> <source src=\"../video/',`chefvideo`.`videoName`,'\"> </video>') AS `videoName`,
                        ...
                        " . ((in_array(14, $assignedmenuedit) || $result['roleId'] == 0) ? "<a href=\"../addchefvideo-',TO_BASE64(`chefvideo`.`chefvideoId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(14, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`chefvideo`.`chefvideoId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "
						...
						FROM `chefvideo`
                        LEFT JOIN `chef` ON `chefvideo`.`chefId`=`chef`.`chefId`
                        LEFT JOIN `cuisines` ON `chefvideo`.`cuisineId`=`cuisines`.`cuisineId`
                        LEFT JOIN `language` ON `chefvideo`.`languageId`=`language`.`languageId`
                       
                        where `chefvideo`.`delete`= '0'
