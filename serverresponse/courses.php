...
$aColumns = array("rowNumber","Actions","name","Description","category","duration","language","price","img");
			...
			SELECT
				...
				(
					SELECT
                        ...
                        `cuisines`.`cuisineName` as category,
                        CONCAT(`course`.`duration`,' hour') as duration,
                        `language`.`name` as language,
                        `course`.`price` as price,
                        

						CONCAT('<img style=\"height: 100px; width:100px;\" src=\"../images/courses/',`course`.`coverImage`,'\" >') as `img`,

						CONCAT('<div class=\"margin-bottom-5\">

						" . ((in_array(18, $assignedmenuedit) || $result['roleId'] == 0) ? "<a href=\"../addcourses-',TO_BASE64(`course`.`courseId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(18, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`course`.`courseId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "
						
						
						
						</div>') AS `Actions`
						
						FROM `course` LEFT JOIN `cuisines` on `course`.`category`= `cuisines`.`cuisineId`
									LEFT JOIN `language` on `language`.`languageId` = `course`.`languageId` where `course`.`delete`= '0'
