...
$aColumns = array("rowNumber","name","price","img","Actions");
	...
	$sTable = "
		(
			SELECT
				...
				(
					SELECT
                        `dietplan`.`name` as name,
                        `dietplan`.`price` as price,


						CONCAT('<img style=\"height: 100px;\" src=\"../images/diet/',`dietplan`.`coverImage`,'\" >') as `img`,

						CONCAT('<div class=\"margin-bottom-5\">
						
						" . ((in_array(15, $assignedmenuedit) || $result['roleId'] == 0) ? "<a href=\"../adddiet-',TO_BASE64(`dietplan`.`dietPlanId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(15, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`dietplan`.`dietPlanId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "

						
						
						</div>') AS `Actions`
						
						FROM `dietplan` where `delete`= '0'
