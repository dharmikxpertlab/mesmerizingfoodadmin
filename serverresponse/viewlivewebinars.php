...
// "gender","languages",
$aColumns = array("rowNumber","Actions","status","chefname","title","description","duration","amount","dateandtime");
		...
		(
			SELECT
				...
				(
					...
					CONCAT(
                        CASE
                        WHEN `cheflive`.`status` = 0
                        THEN 'upcoming live'
                       
                        WHEN `cheflive`.`status` = 1
                        THEN 'live'
                    
                        WHEN `cheflive`.`status` = 2
                        THEN 'live complete'
                        END ) AS `status`,

						
					    
					    `chef`.`fullName` AS chefname,
					    `cheflive`.`title` AS title,
					    `cheflive`.`description` AS description,
					    CONCAT(`cheflive`.`duration`,' hour') AS duration,
					    `cheflive`.`amount` AS amount,
                        CONCAT(`cheflive`.`date`,' ',`cheflive`.`time`) AS dateandtime,


						
						CONCAT('<div class=\"margin-bottom-5\">
						
						" . ((in_array(27, $assignedmenuedit) || $result['roleId'] == 0) ? "<a href=\"../addlivewebinars-',TO_BASE64(`cheflive`.`chefLiveId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(27, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`cheflive`.`chefLiveId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "
						...
						FROM `cheflive` LEFT JOIN  `chef` ON `cheflive`.`chefId` = `chef`.`chefId`  WHERE `cheflive`.`delete` = '0'
