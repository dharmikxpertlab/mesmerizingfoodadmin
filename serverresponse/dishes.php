...
$aColumns = array("rowNumber","Cuisine","Dishe","Actions");
	...
	$sTable = "
		(
			SELECT
				...
				(
					SELECT
                        `cuisines`.`cuisineName` as Cuisine,
                        `dishes`.`dishName` as Dishe,


						CONCAT('<div class=\"margin-bottom-5\">
						
						" . ((in_array(17, $assignedmenuedit) || $result['roleId'] == 0) ? "<a href=\"../adddishes-',TO_BASE64(`dishes`.`dishId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(17, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`dishes`.`dishId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "

						
						
						</div>') AS `Actions`
						
						FROM `dishes` LEFT JOIN `cuisines` on `dishes`.`cuisineId`= `cuisines`.`cuisineId` where `dishes`.`delete`= '0'
