...
$aColumns = array("rowNumber", "Actions", "question", "answer",  "lastUpdate");
			...
			SELECT
				...
				(
					SELECT
                        `faq`.`question`,
                        `faq`.`answer`,
                        CONCAT(`admin`.`adminName`,'<br>At: ',`faq`.`updated`) as `lastUpdate`,
                        
                        

						CONCAT('<div class=\"margin-bottom-5\">

                        " . ((in_array(15, $assignedmenuedit) || $result['roleId'] == 0) ? " <a href=\"../addfaq-',TO_BASE64(`faq`.`faqId`),'/\" style=\"width: 30px;height: 30px;margin-right:5px;\" title=\"Edit\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-edit\"></i></a>" : "") . "
                        
                        " . ((in_array(15, $assignedmenudelete) || $result['roleId'] == 0) ? "<button onclick=\"remove(',`faq`.`faqId`,')\" style=\"width: 30px;height: 30px;\" title=\"Delete\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-trash\"></i></button>" : "") . "
                        
                       
                        
                        </div>') AS `Actions`
						
						FROM `faq` LEFT JOIN `admin` ON `faq`.`adminId`=`admin`.`adminId` where `faq`.`delete`= '0'
