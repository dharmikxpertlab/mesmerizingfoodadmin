...
// "gender","languages",
$aColumns = array("rowNumber","fullName","chefName","callType","amount","status","date");
			...
			SELECT
				...
				(
					SELECT
						...
						`appointment`.`callType` as `callType`,
						`appointment`.`amount` as `amount`,

						CASE
						WHEN `appointment`.`status` = 0
                        THEN 'Booked' 
                        when `appointment`.`status` = 1
                        THEN 'Running'
						WHEN `appointment`.`status` = 2
                        THEN 'Completed' 
                        when `appointment`.`status` = 3
                        THEN 'Reject'
                        END as status,
						
					    CONCAT(`appointment`.`date`,'<br>',`appointment`.`time`)as `date`
						...
						FROM `appointment`
						LEFT JOIN `chef` ON `appointment`.`chefId`=`chef`.`chefId`
