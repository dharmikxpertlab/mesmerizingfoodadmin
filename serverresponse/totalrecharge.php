...
// "gender","languages",
$aColumns = array("rowNumber","fullName","email","totalcoin","totalamount");
			...
			SELECT
				...
				(
					SELECT
						...
						`user`.`email` as `email`,
                        sum(`wallethistory`.`coin`) as `totalcoin`,
                        sum(`wallethistory`.`amount`) as `totalamount`

						FROM `user` 
                        LEFT JOIN `wallethistory` ON `user`.`userId` = `wallethistory`.`id`
						WHERE `user`.`delete` = 0 AND `wallethistory`.`operation` = 'credit' AND `wallethistory`.`type` = 'user' ". $query ."
