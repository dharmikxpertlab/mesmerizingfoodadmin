<?php
$categary = "Master";
$pagename = isset($_GET['edit']) ? "Edit Carousal" : "Add Carousal";
...
if(isset($_GET['edit']))
	...
	if(isset($_POST['save']))
	{
        ...
        $driverimage = 'carousal'. '-'.$imagetimestamp.'.'. $imagefileExt;
        if($imagefileExt==""){
        ...
        if($flag == 1)
		{
			if(isset($_GET['edit']))
			{
				mysqli_query($connection,"UPDATE `slider` SET `type` = 'slider', `images` = '$driverimage' where `imageId` = '$id'") or die(mysqli_error($connection));
				
				$editSuccess=1;
			}
			else
			{
				mysqli_query($connection,"INSERT INTO `slider` SET `type` = 'slider', `images`  = '$driverimage'") or die(mysqli_error($connection));
				$success=1;
			}
        }
    }
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-lg-12" style="text-align: center;">
								...
								<button onClick="window.location.href='../carousal/';" type="button"  class="btn btn-secondary">Cancel</button>
...
<script>
	...
	<?php if(isset($success)) {  $_SESSION['success']=1; ?>
		 ...
		 window.location.href='../addcarousal/';
	
	<?php } else if(isset($editSuccess)) {  $_SESSION['editSuccess']=1; ?>
		
		 window.location.href='../carousal/';
